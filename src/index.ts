 ...
 */
const findFlows = (entries: readonly AuthorizationEntry[], typed: string): AuthorizationEntry[] => {
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const configured = async (key: CredentialKey): Promise<boolean> => {
    try {
  ...
  const listFlows = async (): Promise<CommandResult> => {
  ...
  const runLogin = async (invocation: CommandInvocation, typed: string): Promise<CommandResult> => {
    ...
    const interaction = {
      notify: (notice: AuthorizationNotice): void => {
        ...
        if (notice.url !== undefined) void openPage(notice.url).then(ok => { opened ||= ok })
      },
  ...
  ctx.effect(() => ctx.commands.register({
    ...
    handler: async (invocation: CommandInvocation): Promise<CommandResult> => {
      const typed = invocation.rawInput.trim()
      if (typed === '') return { kind: 'error', text: 'Name the provider to sign out of.' }
      const matches = findFlows(ctx.authorization.list(), typed)
      if (matches.length !== 1) {
